...
const runBridge = ({ scriptPath, modelDir, row }) => {
  ...
  if (result.status !== 0) {
    throw new Error(
      [
        `Notification bridge failed with exit code: ${result.status}`,
...
describe("AI Smoke - predict_notification_bridge.py", () => {
  it("TC_AI_SMOKE_003 returns a valid notification prediction", () => {
    const output = runBridge({
      scriptPath: notificationBridgeScript,
      modelDir: notificationModelDir,
      row: makeNotificationRow(),
    });
    ...
    expect(["both", "limit_warning", "none", "sleep"]).toContain(
      prediction.predictedNotificationType
    );
